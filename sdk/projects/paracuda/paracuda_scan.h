...
__host__ \
 void PARACUDA_##NAME##_run(TYPE* sum, PARACUDA_##TYPE##_struct* array, size_t length, size_t max_threads) \
{\
  if(sum != 0) PARACUDA_##TYPE##_peek(sum, array, length - 1); \
    PARACUDA_##NAME##_upsweep(array, length, max_threads); \
    PARACUDA_##NAME##_downsweep(array, length, max_threads); \
    cutilCheckMsg("Scan"); \
    if(sum != 0) {		   \
      TYPE last;\
      PARACUDA_##TYPE##_peek(&last, array, length - 1);	\
...
{\
    ...
    \
    PARACUDA_##NAME##_run(sum, device_array, length, max_threads);	\
    \
