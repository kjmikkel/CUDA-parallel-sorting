...
void PARACUDA_##T0##_copy_device_device(T0* out, T0* in, size_t length) \
{ \
    cutilSafeCall(cudaMemcpy(out, in, length * sizeof(T0), cudaMemcpyDeviceToDevice)); \
} \
void PARACUDA_##T0##_shallow_copy_host_device(T0* out, T0* in) \
{ \
    cutilSafeCall(cudaMemcpy(&out, &in, sizeof(T0*), cudaMemcpyHostToDevice)); \
} \
void PARACUDA_##T0##_shallow_copy_device_host(T0* out, T0* in) \
{ \
    cutilSafeCall(cudaMemcpy(&out, &in, sizeof(T0*), cudaMemcpyDeviceToHost)); \
} \
T0* PARACUDA_##T0##_shallow_allocate_device() \
{ \
    T0* r; \
    cutilSafeCall(cudaMalloc((void**) &r, sizeof(T0))); \
    return r; \
} \
void PARACUDA_##T0##_shallow_free_device(T0* r) \
{ \
    cutilSafeCall(cudaFree(r)); \
} \
 void PARACUDA_##T0##_peek(T0* out, T0* in, size_t index)	\
{ \
    cutilSafeCall(cudaMemcpy(out, in + index, sizeof(T0), cudaMemcpyDeviceToHost)); \
} \
int PARACUDA_##T0##_equal(T0* a, int ai, T0* b, int bi) \
{ \
    return a[ai] == b[bi]; \
} \
 void PARACUDA_##T0##_add_to_device(T0 value, T0* array, size_t position) { \
  T0 temp;\
  cutilSafeCall(cudaMemcpy(&temp, &(array[position]), sizeof(T0), cudaMemcpyDeviceToHost)); \
  value += temp;\
  cutilSafeCall(cudaMemcpy(&(array[position]),&value, sizeof(T0), cudaMemcpyHostToDevice)); \
}\
...
void PARACUDA_##NAME##_copy_device_device(VECTOR* out, VECTOR* in, size_t length) \
{ \
    VECTOR output; \
    VECTOR input; \
    cutilSafeCall(cudaMemcpy(&input, in, sizeof(VECTOR), cudaMemcpyDeviceToHost));	      \
    cutilSafeCall(cudaMemcpy(&output, out, sizeof(VECTOR), cudaMemcpyDeviceToHost));	      \
    cutilSafeCall(cudaMemcpy(output.N0, input.N0, length * sizeof(T0), cudaMemcpyDeviceToDevice)); \
    cutilSafeCall(cudaMemcpy(output.N1, input.N1, length * sizeof(T1), cudaMemcpyDeviceToDevice)); \
} \
void PARACUDA_##NAME##_shallow_copy_host_device(VECTOR* out, VECTOR* in) \
{ \
    cutilSafeCall(cudaMemcpy(out, in, sizeof(VECTOR), cudaMemcpyHostToDevice)); \
} \
void PARACUDA_##NAME##_shallow_copy_device_host(VECTOR* out, VECTOR* in) \
{ \
    cutilSafeCall(cudaMemcpy(out, in, sizeof(VECTOR), cudaMemcpyDeviceToHost)); \
} \
...
void PARACUDA_##NAME##_peek(NAME* r, VECTOR* v, size_t index)	\
{ \
    VECTOR temp; \
    cutilSafeCall(cudaMemcpy(&temp, v, sizeof(VECTOR), cudaMemcpyDeviceToHost));     \
    cutilSafeCall(cudaMemcpy(&(r->N0), temp.N0 + index, sizeof(T0), cudaMemcpyDeviceToHost)); \
    cutilSafeCall(cudaMemcpy(&(r->N1), temp.N1 + index, sizeof(T1), cudaMemcpyDeviceToHost)); \
} \
...
{ \
    VECTOR temp; \
    cutilSafeCall(cudaMemcpy(&temp, out, sizeof(VECTOR), cudaMemcpyDeviceToHost)); \
    cutilSafeCall(cudaMemcpy(&(out->N0), &(in->N0), length * sizeof(T0), cudaMemcpyHostToDevice)); \
    cutilSafeCall(cudaMemcpy(&(out->N1), &(in->N1), length * sizeof(T1), cudaMemcpyHostToDevice)); \
    cutilSafeCall(cudaMemcpy(&(out->N2), &(in->N2), length * sizeof(T2), cudaMemcpyHostToDevice)); \
} \
void PARACUDA_##NAME##_copy_device_host(VECTOR* out, VECTOR* in, size_t length) \
{ \
    VECTOR temp; \
...
{ \
    ...
    cutilSafeCall(cudaMemcpy(output.N0, input.N0, length * sizeof(T0), cudaMemcpyDeviceToDevice)); \
    cutilSafeCall(cudaMemcpy(output.N1, input.N1, length * sizeof(T1), cudaMemcpyDeviceToDevice)); \
    cutilSafeCall(cudaMemcpy(output.N2, input.N2, length * sizeof(T2), cudaMemcpyDeviceToDevice)); \
} \
void PARACUDA_##NAME##_shallow_copy_host_device(VECTOR* out, VECTOR* in) \
{ \
    cutilSafeCall(cudaMemcpy(out, in, sizeof(VECTOR), cudaMemcpyHostToDevice)); \
...
{ \
    ...
    cutilSafeCall(cudaMemcpy(&(r->N0), temp.N0 + index, sizeof(T0), cudaMemcpyDeviceToHost)); \
    cutilSafeCall(cudaMemcpy(&(r->N1), temp.N1 + index, sizeof(T1), cudaMemcpyDeviceToHost)); \
    cutilSafeCall(cudaMemcpy(&(r->N2), temp.N2 + index, sizeof(T2), cudaMemcpyDeviceToHost)); \
} \
int PARACUDA_##NAME##_equal(VECTOR* a, int ai, VECTOR* b, int bi) \
{ \
