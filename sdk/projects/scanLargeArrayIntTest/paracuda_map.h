...
__global__ \
 void PARACUDA_##NAME##_kernel(PARACUDA_##OUTPUT##_struct* output, PARACUDA_##INPUT##_struct* input) \
{ \
...
{ \
    if(length <= max_threads) {\
        ...
        PARACUDA_##NAME##_kernel<<<grid, block>>>(output, input);\
        return;\
    }\
    int numBlocks = length / max_threads;\
    int numThreadsPerBlock = max_threads;\
    dim3 dim(numThreadsPerBlock);\
    PARACUDA_##NAME##_kernel<<<numBlocks, dim>>>(output, input);\
